...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("--trail-activation-r", type=float, default=0.5, help="Gate the trailing stop behind this R-multiple of unrealized profit. 0 = trail is live the moment the position fills (legacy, wicks out early). 0.5 = trail only starts tracking once the trade reaches +0.5R. Prevents wick-out losses on entry pullbacks. Backtest validated: 0.5 doubled expectancy (+0.087R -> +0.182R) and win rate (33%% -> 45%%) vs the always-active default.")
